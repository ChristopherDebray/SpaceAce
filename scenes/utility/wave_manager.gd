...
const WAVES: EnemyWaves = preload("res://ressources/waves.tres")
...
func set_random_path_index() -> void:
	...
	# last_path_index will be equal to the last used wave, we delete it from index to not have the same wave
...
func spawn_wave() -> void:
	...
	for i in range(0, wave.get_number()):
		...
		# THis will make the gap timer between each enemy spawn
...
func create_enemy(wave: EnemyWave) -> BaseEnemy:
	var new_en: BaseEnemy = wave.get_enemy_scene().instantiate()
